...
import { Box, VStack, Text, Image,Progress, Flex } from '@chakra-ui/react';
...
const WhyUs = () => {
  ...
  return (
      ...
      <Flex  margin={{base:'2rem 1.5rem',md:'4rem 5rem'}} mt={{base:'2rem',md:'8rem'}} justifyContent='center' gap='5rem'flexDirection={{ base: 'column', md: 'row' }}>
        <VStack align="start" width='auto' spacing={2} flex={1}>
          <Text fontSize={{base:'2.4rem',md:'2.7rem'}} color='about_heading' mb={{base:'1rem',md:'2rem'}} fontWeight="bold">
            Why Choose US
          </Text>
          <Text fontSize={{base:'1.7rem',md:'2rem'}} textAlign={{base:'center',md:'left'}} color="gray.500">
            {AboutUsData.whyUs[currentIndex].subheading}
          </Text>
          <Text textAlign={{base:'center',md:'left'}} width={{base:'100%',md:'90%'}}>{AboutUsData.whyUs[currentIndex].description}</Text>
        ...
        <Box  position="relative"   borderRadius="md">
          ...
          {currentIndex < AboutUsData.whyUs.length - 1 && (
            <Image
              src={`${process.env.PUBLIC_URL}/Assets/Images/${AboutUsData.whyUs[currentIndex+1].image}`}
